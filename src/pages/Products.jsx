...
export default function Products() {
  ...
  const {
    currentPage, totalPages, perPage, pageItems,
    totalItems, listTopRef, goToPage, resetToFirstPage,
    next, prev, changePerPage, hasPrev, hasNext,
      ...
      <div className="container">
        ...
        <div className="products-header">
          <div className="section-heading">
            ...
            <p>
              Browse our range of professional-grade security and networking equipment.
              All products are available for enquiry &mdash; no online checkout.
            </p>
          </div>
          <div className="products-toolbar">
            <input
              ...
              onChange={(e) => {
                ...
                resetToFirstPage(); // ← no scroll, keeps focus on input
              }}
            />
          ...
          {productCategories.map((cat) => (
            <button
              ...
              onClick={() => {
                ...
                resetToFirstPage(); // ← no scroll for category change either
              }}
            >
              {cat}
            </button>
        ...
        <p className="products-count" aria-live="polite">
          {totalItems === 0
            ? "No products found."
            : `Showing ${(currentPage - 1) * perPage + 1}–${Math.min(currentPage * perPage, totalItems)} of ${totalItems} product${totalItems !== 1 ? "s" : ""}`}
        </p>
          ...
          {pageItems.length === 0 ? (
            ...
            pageItems.map((product) => (
              <ProductCard
                ...
                fallbackIcon={CATEGORY_ICONS[product.category] || "🔒"}
              />
            ))
          )}
        </div>
...
function ProductCard({ product, image, fallbackIcon }) {
